...
def eval_one_epoch(epoch, dataloader, model, device, train_dir, log_file):
    with torch.no_grad():
        ...
        message = "total Threshold: {:.3f} =====> Evaluation IOU: {:.4f}; F1_score: {:.4f}; AUC: {:.4f}; ACC: {:.4f}".format(
            0.4, epoch_iou, epoch_f1, epoch_auc, epoch_acc)
...
def train_eval_model(opts):
    ...
    shutil.copy("./models/segnet_modified.py", os.path.join(model_dir, "segnet_modified.py"))
    shutil.copy("./trainer_segnet.py", os.path.join(model_dir, "trainer_segnet.py"))
...
if __name__ == '__main__':
    ...
    opts["gpu_list"] = "0,1,2"
    opts["log_dir"] = "./train_log/nucleus_train_segnet_modified_20210425_iouloss_0.1_10_norm"
