...
def eval_one_epoch(epoch, dataloader, model, device, train_dir, log_file):
    with torch.no_grad():
        ...
        message = "total Threshold: {:.3f} =====> Evaluation IOU: {:.4f}; F1_score: {:.4f}; AUC: {:.4f}; ACC: {:.4f}".format(0.5 , epoch_iou, epoch_f1,epoch_auc,epoch_acc)
        print("==> %s" % (message))
        with open(log_file, "a+") as fid:
            fid.write('%s\n' % message)
...
def train_eval_model(opts):
    ...
    unet_model.cuda()
    unet_model = nn.DataParallel(unet_model,device_ids=[0,1,2])
...
if __name__ == '__main__':
    ...
    opts["gpu_list"] = "0,1,2"
    opts["log_dir"] = "./train_log/nucleus_new_selected_best_agnet_20210510"
